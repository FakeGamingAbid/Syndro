...
class _TransferProgressScreenState extends ConsumerState<TransferProgressScreen>
  ...
  void _calculateSpeed() {
    ...
    if (mounted) {
      setState(() {
        _speed = bytesPerSecond.toDouble();
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        if (_currentTransfer?.status == TransferStatus.transferring ||
            _currentTransfer?.status == TransferStatus.connecting ||
          ...
          leading: IconButton(
            ...
            onPressed: () {
              if (_currentTransfer?.status == TransferStatus.completed ||
                  _currentTransfer?.status == TransferStatus.failed ||
                  _currentTransfer?.status == TransferStatus.cancelled) {
                Navigator.of(context).pop();
              } else {
                _cancelTransfer();
              }
