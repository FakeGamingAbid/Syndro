...
class Device extends Equatable {
  ...
  /// Get the subnet portion of the IP address (e.g., "192.168.1" from "192.168.1.100")
  String? get subnet {
    if (ipAddress.isEmpty || ipAddress == '0.0.0.0') return null;
    final parts = ipAddress.split('.');
    if (parts.length != 4) return null;
    return '${parts[0]}.${parts[1]}.${parts[2]}';
  }

  /// Check if this device is on the same subnet as another IP address
  bool isOnSameSubnetAs(String otherIp) {
    final mySubnet = subnet;
    if (mySubnet == null) return false;
    
    final otherParts = otherIp.split('.');
    if (otherParts.length != 4) return false;
    
    final otherSubnet = '${otherParts[0]}.${otherParts[1]}.${otherParts[2]}';
  ...
  factory Device.fromJson(Map<String, dynamic> json) {
    ...
    final port = json['port'] as int? ?? AppConfig.defaultTransferPort;
