...
class FileService {
  ...
  /// FIX #8: Validate that a path is within an allowed directory
  /// Now resolves symlinks to prevent symlink attacks
  ...
  /// FIX #8: Get download directory - PROPERLY HANDLES ALL ANDROID DEVICES
  Future<String> getDownloadDirectory() async {
    try {
      if (Platform.isAndroid) {
        // FIX #8: Try multiple paths in order of preference
        final possiblePaths = [
          '/storage/emulated/0/Download/Syndro',
          '/sdcard/Download/Syndro',
        ];

        // First, try the standard Download path
        ...
        // FIX #8: Fallback to app-specific external storage
        try {
        ...
        // FIX #8: Last resort - app documents directory
        try {
        ...
        // FIX #8: This should never happen, but if it does, throw instead of returning unusable path
        throw FileServiceException(
        ...
        if (userProfile != null && userProfile.isNotEmpty) {
          // FIX #8: Use single backslash (path library handles this)
          final syndroDir =
