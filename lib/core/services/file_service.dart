...
class FileService {
  ...
  Future<int> getFileSize(String filePath) async {
    try {
      ...
      if (errorMsg.contains('permission denied') ||
        ...
        throw FileServiceException(
          'This file is in use by another app',
          code: 'FILE_LOCKED',
          originalError: e,
        );
      }
  ...
  /// Throws [FileServiceException] if file is empty
  Future<void> validateFileNotEmpty(String filePath) async {
    final size = await getFileSize(filePath);
    if (size == 0) {
      throw FileServiceException(
        'This file is empty and cannot be transferred',
        code: 'EMPTY_FILE',
      );
    }
  }

  /// Validate that a folder doesn't have too many files
  /// Returns a warning message if folder has 500+ files, null otherwise
  Future<String?> checkLargeFolder(String folderPath) async {
    try {
      ...
      await for (final entity in dir.list(recursive: true)) {
        if (entity is File) {
          count++;
          if (count > 500) {
            return 'Large folders may take a moment to prepare';
          }
        }
      }
      return null;
