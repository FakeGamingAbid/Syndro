...
(
    ...
    echo.
    echo ; Syndro - Right-Click "Send with Syndro" Context Menu
    echo.
    echo ; Add "Send with Syndro" to file context menu
    echo [HKEY_CURRENT_USER\Software\Classes\*\shell\SyndroSend]
    echo @="Send with Syndro"
    echo "Icon"="\"!SYNDRO_PATH:\=\\!\",0"
    echo.
    echo [HKEY_CURRENT_USER\Software\Classes\*\shell\SyndroSend\command]
    echo @="\"!SYNDRO_PATH:\=\\!\" \"%%1\""
    echo.
    echo ; Add "Send with Syndro" to folder context menu
    echo [HKEY_CURRENT_USER\Software\Classes\Directory\shell\SyndroSend]
    echo @="Send with Syndro"
    echo "Icon"="\"!SYNDRO_PATH:\=\\!\",0"
    echo.
    echo [HKEY_CURRENT_USER\Software\Classes\Directory\shell\SyndroSend\command]
    echo @="\"!SYNDRO_PATH:\=\\!\" \"%%1\""
    echo.
    echo ; Add "Send with Syndro" to directory background (right-click in folder)
    echo [HKEY_CURRENT_USER\Software\Classes\Directory\Background\shell\SyndroSend]
    echo @="Send with Syndro"
    echo "Icon"="\"!SYNDRO_PATH:\=\\!\",0"
    echo.
    echo [HKEY_CURRENT_USER\Software\Classes\Directory\Background\shell\SyndroSend\command]
    echo @="\"!SYNDRO_PATH:\=\\!\" \"%%V\""
) > "%REG_FILE%"
